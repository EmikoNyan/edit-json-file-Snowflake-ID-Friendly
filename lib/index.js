    ...
    _createClass(JsonEditor, [{
        ...
        value: function set(path, value, options) {
            ...
            const isNumericString = str =>
                typeof str === "string" && /^[0-9]+$/.test(str);

            const toPathArray = pathStr => {
                if (typeof pathStr !== "string" || !pathStr.includes(".")) return pathStr;
                const parts = pathStr.split(".");
                return parts.map(p => (isNumericString(p) ? String(p) : p));
            };
            ...
            if (typeof path === "object") {
                ...
                const pathArg = toPathArray(path);

                if (this.options.ignore_dots || isNumericString(path)) {
                this.data[path] = value;
                } else {
                setValue(this.data, pathArg, value, options);
                }
            }
